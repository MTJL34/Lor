...
$constellations = @(Get-ExportArrayObject -FilePath 'data/Constellation_Number.js' -ExportName 'Constellation_Number')
if ($constellations.Count -eq 1 -and $constellations[0] -is [System.Array]) {
    $constellations = @($constellations[0])
}

$champions = @(Get-ExportArrayObject -FilePath 'data/Champion.js' -ExportName 'Champion')
...
Set-Content -Path 'data/poc_constellation.sql' -Value $out -Encoding UTF8
Write-Output 'SQL generated: data/poc_constellation.sql'
